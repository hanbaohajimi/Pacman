...
_user_closed = False     # 用户点击关闭按钮


class GraphicsClosedByUser(Exception):
    """用户关闭游戏窗口时抛出，用于正常结束对局而非 sys.exit。"""
    pass


def user_closed_graphics():
    return _user_closed


def _raise_if_user_closed():
    if _user_closed:
        raise GraphicsClosedByUser()
...
def sleep(secs):
    ...
    if _root_window is None:
        ...
        _raise_if_user_closed()
        _root_window.update_idletasks()
        _root_window.after(int(1000 * secs), _root_window.quit)
        _root_window.mainloop()
        _raise_if_user_closed()

def sleep_light(secs):
    """动画帧间等待：避免每帧嵌套 mainloop（Windows 下易明显卡顿）。"""
    global _root_window
    if _root_window is None:
        time.sleep(secs)
        return
...
def begin_graphics(width=640, height=480, color=formatColor(0, 0, 0), title=None):
    ...
    global _root_window, _canvas, _canvas_x, _canvas_y, _canvas_xs, _canvas_ys, _bg_color, _user_closed
...
def _destroy_window(event=None):
    """关闭按钮：退出嵌套 mainloop 并结束当前对局，不 sys.exit 整个进程。"""
    global _root_window, _canvas, _user_closed
    _user_closed = True
    if _root_window is not None:
        try:
            _root_window.quit()
...
def pump_graphics_events():
    """处理关闭按钮、键盘等窗口事件（update_idletasks 无法处理 WM_DELETE_WINDOW）。"""
    global _root_window
    if _root_window is None:
        return
    try:
...
def end_graphics():
    global _root_window, _canvas, _mouse_enabled, _user_closed
    try:
        try:
            sleep(1)
...
def refresh():
    _raise_if_user_closed()
    if _canvas is not None and _root_window is not None:
        pump_graphics_events()
        try:
