...
class GameStateData:
    ...
    def deepCopy( self ):
        ...
        # 迷宫布局整局不变，不必每步复制（大地图幽灵回合会反复 deepCopy）
...
class Game:
    ...
    def _getObservation(self, agent):
        """只读智能体（键盘/随机幽灵）直接读当前状态，避免大地图每步整盘 deepCopy。"""
        if getattr(agent, 'READ_ONLY_OBSERVATION', False):
            return self.state
    ...
    def run( self ):
        ...
        try:
            self._runGameLoop(agentIndex, numAgents)
        except graphicsUtils.GraphicsClosedByUser:
            self.gameOver = True
            graphicsUtils.end_graphics()
            return

    def _runGameLoop(self, agentIndex, numAgents):
        while not self.gameOver:
            graphicsUtils.pump_graphics_events()
            ...
            if 'observationFunction' in dir( agent ):
                ...
                if self.catchExceptions:
                    try:
                        ...
                        try:
                            start_time = time.time()
                            observation = timed_func(self._getObservation(agent))
                        except TimeoutFunctionException:
                            skip_action = True
                    ...
                    observation = agent.observationFunction(self._getObservation(agent))
                self.unmute()
            else:
                observation = self._getObservation(agent)
